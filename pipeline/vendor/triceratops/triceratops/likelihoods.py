...
# pytransit 2.2.0 predates several removals in numpy/scipy/setuptools. Inject
# the shims before the pytransit import runs; each guard is a no-op where the
# real name still exists, so a compatible env is untouched. Without them,
# `import triceratops` dies deep inside pytransit with an opaque ImportError.
#
...
# pkg_resources                     : removed in setuptools 81; pytransit's
#                                     contamination module imports only
...
# find_spec raises on a spec-less stub, so short-circuit once it is installed.
if "pkg_resources" not in sys.modules and importlib.util.find_spec("pkg_resources") is None:
    ...
    _stub.resource_filename = _resource_filename  # type: ignore[attr-defined]
    _stub.get_distribution = lambda name: types.SimpleNamespace(version="0")  # type: ignore[attr-defined]
