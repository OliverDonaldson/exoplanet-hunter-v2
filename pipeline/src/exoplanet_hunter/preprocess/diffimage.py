...
DV publishes one difference image per sector as a pixel list sized to that
target's aperture; a branch needs a fixed shape. The re-grid is exact, not a
resampling — a stamp's pixel list fills its bounding box completely, and
`regrid_stamp` asserts that rather than assuming it. Nothing is interpolated:
that would make one grid cell a different number of CCD pixels per target, and
the branch's subject is *where* flux moved.

Three states look like zeros and are kept apart — no DV product, a sector DV
declined to measure, and one measured and genuinely featureless. Only the third
is evidence and only it carries `present` 1. Channel 2 is the star's own pixel
position, added for Phase 1; statistics: `docs/experiments/phase-1-build-4-2d.md`.
"""
...
#: Side of the fixed grid, in CCD pixels: the smallest grid on which no stamp
#: loses its peak pixel, which is the pixel the diagnostic is about. A wider grid
#: buys padding. Measured over the archive; see
#: `docs/experiments/stage-09-difference-image.md`.
DIFF_GRID = 17
...
def _target_marker(
    ...
    ExoMiner writes a hard one-hot at the rounded target pixel and recovers the
    sub-pixel part by up-sampling the stamp first. We deposit the mass bilinearly
    instead, because `DIFF_GRID` is one CCD pixel per cell by construction and
    up-sampling would undo the refusal to resample the module rests on. At an exact
    pixel centre it reduces to ExoMiner's one-hot; off-centre the ratio between the
    four weights *is* the offset, and the rounding residual is uniform, so a hard
    one-hot would discard a full pixel of position on every stamp.

    The mass is not renormalised when part of it falls off the grid: a marker
    summing to under one says the star is near the frame edge, which is true, and
    rescaling would state a position the data does not support. None is returned
    only when no mass lands at all. Measurements:
    `docs/experiments/phase-1-build-4-2d.md`.
    """
...
def regrid_stamp(image: DVDifferenceImage) -> np.ndarray | None:
    ...
    Returns `(DIFF_GRID, DIFF_GRID, 4)` = `[difference, out-of-transit, target,
    present]`. Both flux channels are divided by the out-of-transit peak, so a
    difference reads as a fraction of the star's own brightness and is comparable
    between a bright star and a faint one; normalising each channel by its *own*
    peak would send every difference image to 1.0 and destroy the depth.

    The `target` channel is the star's own sub-pixel position (`_target_marker`),
    which is what makes a centroid *offset* computable from the branch's inputs.

    None means "not a measurement" — no pixels, a sector DV declined, no usable
    out-of-transit scale, or a target position DV left undefined or off-frame — and
    the caller must encode it as absent rather than as a flat stamp.
    """
...
def build_difference_views(
    ...
    *,
    ...
    Returns `(stamps, quality)`: stamps `(max_sectors, grid, grid, 4)` with unused
    slots zero, quality `(max_sectors, 2)` as `[quality_metric, present]`.

    Sectors are kept highest quality first, so a target with more sectors than slots
    keeps its most trustworthy ones. That is not free — the retained distribution
    then depends on how many sectors a target had — but keeping the first eight by
    sector number would feed the branch images DV itself flags as untrustworthy.
    ...
    reading like one, and it is only ever a relative weight between one target's own
    sectors, so rescaling would invent a range DV does not claim.
    """
