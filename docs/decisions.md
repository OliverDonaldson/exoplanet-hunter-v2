...
| 2026-09-19 | **The pAUC one-sided veto is withdrawn, not implemented.** The 2026-09-14 row below records it as adopted; measured on 2026-09-19 it appears nowhere in `pipeline/src`, `evaluate_promotion` has no pAUC term, no test covers one, and a candidate whose pAUC collapses 0.754 -> 0.300 promotes with no alarm. It is also not *reported* — `SliceMetrics` never emitted it. Not implemented, not reported, claim withdrawn, including from the console's user-facing copy. The ±0.036 trigger #105 called "close to inert" is the 5-against-1 allocation of the re-baseline row above, not a property of pAUC — at 5v5 it is ±0.0216 — so the reason not to build it is cost at the current allocation. **The relevance guard is therefore unfilled**, and P2.1 §4 shows no label-based metric can fill it; injection-recovery is the only instrument that sets its own n there | [P2.1 note](experiments/p2-1-power-analysis-2026-09-14.md); [#105](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/105) |
| 2026-09-19 | **The champion is re-baselined at M=5, with K2 — reversing the refusal recorded in `promotion.py`'s `ACKNOWLEDGED_ALARMS`.** That comment said re-baselining "is not taken, because it would move the comparison every past result was read against". The cost is now accepted: the champion carries no variance block, so `decision_floor` borrows `POOLED_SEED_SD` over `n_inc = 1` and the allocation the gate faces is 5 against 1, not the 5-against-5 the published MDEs assume — **0.0199 dual-view, not 0.0126**. M=3 reaches only 0.0140, still 11% short of the figure pre-registrations quote, so the re-baseline is at M=5. Re-scoring cannot substitute: `seed_sd` is written only by the trainers. Every comparison read against the borrowed-prior floor is superseded | [P2 re-baseline](experiments/p2-champion-rebaseline-2026-09-19.md); [#114](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/114) |
| 2026-09-15 | **The weekly refresh is a calibrator, not a challenger — executed.** `--promote` is removed from the flow, so the loop gates, logs its verdict and reports, and never writes `models/registry.json`. The re-scoping was recorded in `PLAN.md` §5 on 2026-09-07 and the code did not follow; after P2.1 landed on 2026-09-14 that gap meant an unattended run would have promoted on **recall @1% FPR**, the metric P2.1 had just demoted for an MDE of 0.1222. The PROMOTE headline and the console's "the promotion criterion" copy are corrected in the same change | [P2.1](experiments/p2-1-power-analysis-2026-09-14.md); `PLAN.md` §6 |
| 2026-09-14 | **The promotion gate reads ROC-AUC, not partial AUC.** P2.1's standing recommendation is half accepted: recall @1% FPR is demoted from gating (MDE 0.1222 at 5 members, larger than the span of the record it was deciding), but pAUC FPR≤0.1 is rejected as its replacement — ROC-AUC detects the same degradation at 1.9x pAUC's z, and where pAUC should have won, on a change confined to the follow-up region, the two are within noise and both insignificant. pAUC becomes a one-sided veto, which keeps the relevance guard without the power penalty | [P2.1](experiments/p2-1-power-analysis-2026-09-14.md) |
| 2026-09-14 | **P2.4 is promoted ahead of P2.2 and P2.3 in priority.** No metric available to this project detects a change confined to the top 5–20% of the ranking, so the region the console exists to order is the region the labelled evidence cannot see. Only injection-recovery sets its own n there. §5's order is changed on this finding rather than silently re-planned, per #78 | [P2.1](experiments/p2-1-power-analysis-2026-09-14.md) §4 |
| 2026-09-14 | **P2.2's interval must carry the seed term.** Seed sd exceeds sampling sd for every candidate metric — ROC-AUC 0.0093 against 0.0021 — so a paired bootstrap on the contrast measures the smaller component and under-states what a gate faces. Members required before rows bind: ~20 for ROC-AUC, ~11 for pAUC, ~2 for recall @1% FPR | [P2.1](experiments/p2-1-power-analysis-2026-09-14.md) §1 |
| 2026-09-14 | **The clean run lifts the Kepler cap and includes K2** (#78 part B, full scope), at roughly 30–40 h compute across several overnight runs, one process per CV run. Ollie's call, taken 2026-09-14; the alternative was keeping the 1,250/1,250 subsample and restating the exclusion in writing | [#78](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/78) |
