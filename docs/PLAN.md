...
| **P2.2f** · pre-registered 2026-09-21 | **The recall guard objects to a recall improvement.** `abs(margin)` on a two-signed margin, so improving recall by about one floor blocks the promotion while half a floor or two floors do not. Blocks 7.8% of candidates at a 5-se AUC margin. One line at the call site; #128 untouched. [#131](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/131), [p2-recall-one-sided-2026-09-21.md](experiments/p2-recall-one-sided-2026-09-21.md) | A guard that fires only on a regression; no rejection changed | ≈1 h, no training |
