...
| **P2.2g** · pre-registered 2026-09-24 | **The AUC guard promotes only past its floor.** The AUC call's band becomes `(0, floor]`; recall keeps its one-sided band (#131). Size at 5 v 5 falls 26.7% → 9.0% on today's floor, and to 1.8% once #123 lands, so #123 follows it. [#128](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/128), [p2-auc-floor-rule-2026-09-24.md](experiments/p2-auc-floor-rule-2026-09-24.md) | The implementation reproduces the costed rule in every cell; no verdict promotes on a margin that did not clear its floor. Power is read against the **81.2% ceiling** the Brier and ECE constants impose ([#134](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/134)), so P2.2d's "power matching the MDE" is unreachable by any AUC rule | ≈2 h, no training |
| ~~P2.2f~~ **DONE 2026-09-24** | **The recall guard fires only on a regression.** `abs(margin)` on a two-signed margin had it block a recall gain of about one floor while half a floor or two floors passed. One line at the call site; `unresolved_against` and #128 untouched. [#131](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/131), [pre-registration](experiments/p2-recall-one-sided-2026-09-21.md), [result](experiments/p2-recall-one-sided-result-2026-09-24.md) | Not falsified: no REJECT and no negative-margin verdict moves, PROMOTE at 5 se 71.6% → 79.2%. **The MDE prediction is falsified**, 40.5% against 42–45%. Size 30.4% → 33.7%, as pre-registered; power at the MDE 40.5% against the 80% it asserts, which is #128's to fix | done |
| **P2.2e** · measured 2026-09-20, **falsified** | **The band fix does not work, and the recall guard is the binding constraint.** Power at one MDE measured 1.6%; 98–100% of UNRESOLVED verdicts come from the recall guard, not the AUC guard. `unresolved_against` serves two different questions. Reverted, nothing in `pipeline/src` changed. [p2-unresolved-band-result-2026-09-20.md](experiments/p2-unresolved-band-result-2026-09-20.md) | Size **and** power reported together, which is what caught it | done |
| **P2.2d** · pre-registered 2026-09-20, **blocks P2.2b** | **The UNRESOLVED band has a hole under it: a positive margin below `floor/1.5` promotes.** The rule is not monotone — 2 se promotes 38.8% against 49.5% at 1 se — and gives 49% power at the MDE published as 80%. Until it is closed, a more accurate floor is a worse gate. [#128](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/128), [p2-unresolved-band-2026-09-20.md](experiments/p2-unresolved-band-2026-09-20.md) | A monotone rule; measured size in single figures; power at one MDE matching what that MDE asserts | ≈2 h, no training |
| **P2.2c** · measured 2026-09-20 | **The gate's size under a true null: 28.3% false-PROMOTE, 1.1% under `strict`, which no caller passes.** The fall-through is the UNRESOLVED band's geometry, not the floor's width — and widening the floor makes it worse, which falsified P2.2b's condition 3. [#115](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/115), [p2-gate-size-2026-09-20.md](experiments/p2-gate-size-2026-09-20.md) | Size measured and reported beside power, per Chagas §4.2 | done, 6 s |
| **P2.2b** · pre-registered 2026-09-20, **blocks every gated item below** | **The gate's AUC floor is built on the wrong population.** `decision_floor`'s candidate term is all-mission (`train.py:551`); the margin it bounds is the TESS slice. Measured on the re-baseline: 0.00225 against **0.00372**, so every floor computed so far understated the candidate's noise by 1.65x. [#123](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/123), [p2-seed-sd-population-2026-09-20.md](experiments/p2-seed-sd-population-2026-09-20.md) | A floor whose variance is measured on the statistic the gate reads; every recorded verdict re-checked; the gate's size measured under a true null before and after ([#115](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/115)) | ≈3 h, no training |
