...
rate limiting on `/score`.

**`HANDOVER.md` is deliberately *not* retired here.** It was, on 2026-08-09, and
reverted the same day: the file already opens with a SUPERSEDED banner, so the
tidying bought little, and while the project is still in flight a 2,077-line
record of how things were built is more use in the obvious place than in an
archive directory. It is a finishing task and has moved there.
