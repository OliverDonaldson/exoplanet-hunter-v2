...
| **The CV grouping is inert; the dedup upstream is what prevents leakage** | `StratifiedGroupKFold` is passed `tic_id`, but `labels.parquet` is 5,812 rows on 5,812 distinct `tic_id` — max group size 1, so it partitions identically to `StratifiedKFold`. The project is leak-free via the label builder's one-row-per-host emit, not via the splitter. The guard is worth keeping for a future TCE-level set (#37) but earns no credit today, and the dedup costs every multi-planet system's extra rows | `labels.parquet`, verified 2026-09-10; [report §5.1](report.md) |
| **The fold window is specified in phase and applied in days** | `build_views` computes its windows in phase units and `fold_to_profile` passes them to `lc.fold`, which lightkurve returns in **days**. The global view therefore spans ±0.5 d — 1/P of the phase, measured at **28.6% for P=3.5 d, 10.0% at P=10, 3.3% at P=30** — so above P=1 d it cannot reach phase 0.5 and cannot carry the secondary eclipse the About page advertises. The local view spans **3/P durations**, not 3: 0.10 durations at P=30 d, entirely inside the transit, leaving no baseline for `_normalise` to scale against. The shipped local views' baseline fraction correlates with log period at **+0.4351** (n=5,321), which a period-independent ±3-duration window cannot produce. Not fixed: reconciling the units changes every model input and invalidates the served run's training set | [ML workflow audit](experiments/ml-workflow-audit-2026-09-14.md) §3.1, 2026-09-14; `pipeline/tests/test_view_window_units.py` |
| **The Kepler set is a configured 45% subsample** | `catalog.py::_stable_sample` md5-ranks by `tic_id` and takes the first n, giving exactly 1,250/1,250. **Corrected 2026-09-14**: the eligible pool measured from the refresh's own log is **1,945 confirmed and 3,719 FPs**, not the 2,748 and 3,813 first recorded — the confirmed figure was out by 803 rows. 2,500 of 5,664 eligible rows are used, **44.1%**, so the 45% headline survives and the component counts do not. Unbiased and refresh-stable, but it is less data than was available, in a project §6.4 shows is power-starved | `catalog.py:440`; [benchmark comparability audit](experiments/benchmark-comparability-2026-09-10.md) |
| **Behind the published state of the art on the metric promotion is decided on** | Of six candidate comparisons, five fail on population, protocol or metric definition and cannot be stated either way; a three-refuter panel overturned all six verdicts formally, and NOT COMPARABLE holds substantively on five. The sixth is comparable and unfavourable: ROC monotonicity makes a published point below 1% FPR a lower bound on recall @1% FPR, giving **ExoMiner ≥ 0.936 against our Kepler 0.8129** and **ExoMiner++ ≥ 0.951 against our TESS 0.3113** — about 12 and 64 points behind. An earlier version of this row claimed the metric had no published counterpart and credited the TESS AUC gap to a shared protocol; both claims were broken by the panel | [benchmark comparability audit](experiments/benchmark-comparability-2026-09-10.md) §8, panel 2026-09-10 |
...
| **Seed noise is larger than sampling noise, for every metric** | The record reads margins against bootstrap sds and seed floors estimated from three draws. Measured across the three members of one Phase 1 arm — identical data, identical architecture — the seed sd exceeds the paired-bootstrap sd for every candidate metric: ROC-AUC **0.0093 against 0.0021**, pAUC FPR≤0.1 0.0230 against 0.0071, recall @1% FPR 0.0467 against 0.0383. A confidence interval built from the bootstrap alone measures the smaller half. Members needed before rows become the binding constraint: ~20 for ROC-AUC, ~11 for pAUC, ~2 for recall @1% FPR — so the more stable the metric, the more members it needs | [P2.1](experiments/p2-1-power-analysis-2026-09-14.md) §1, 2026-09-14 |
| **The MDE belongs to an architecture, and the first one published was the wrong one** | P2.1's 0.0131 came from one arm's three members (2 df) on the *branch* architecture, and carried one seed term where a contrast needs two. Pooled over the 15 multi-member runs on disk, the TESS-slice seed sd is **0.0108 for `cnn_branches` (26 df) and 0.0062 for `cnn_dualview` (4 df)** — per-run values span 0.0032–0.0193, which is what 2 df looks like. The bar at 5 members/fold is **0.0201 branch, 0.0126 dual-view**. A dual-view figure read under branch numbers, or the reverse, is the category error `index.md` rule 7 forbids. The dual-view estimate is still only 4 df; P2.6 retires that | [Stage 1](experiments/p2-stage-1-instrument-2026-09-17.md) §1–2, 2026-09-17 |
| **Every floor the gate has read was understated ~1.9x** | `decision_floor` divided the borrowed champion variance by the *candidate's* member count where 4.1b specifies the champion's. The served champion has always been a single-member run, so the error applied to every decision the project has made. Corrected 2026-09-17; all three promotion logs on disk re-checked and **no recorded verdict changes** — the correction widens floors, so it can only move a REJECT toward UNRESOLVED | [Stage 1](experiments/p2-stage-1-instrument-2026-09-17.md) §3, 2026-09-17 |
| **The gate is over-sized until #123 lands, and its power is capped at 81%** | Simulated over `evaluate_promotion` with both arms drawn at the TESS per-member sds, 5 v 5: after #128 a candidate identical to the champion is promoted **9.0%** of the time on the floor the trainer writes today, and **1.8%** once #123 measures the seed sd on the gate slice (nominal 2.3%). Power at the 2.802-se MDE is **76.6%**, **63.7%** after #123 — not the 80% every published MDE asserts — and no AUC rule can pass **81.2%**, because the Brier and ECE tolerances are constants that reject 11.3% of identical candidates on noise alone; against a single-member champion that is 32.3% and power caps at 63.6% (#134). The weekly refresh passes `--strict`, which holds size at or under 0.3%; the manual CLI defaults off and is the only path that promotes. A candidate whose summary carries no AUC `seed_sd` has no floor at all and promotes on any positive margin | [#128 result](experiments/p2-auc-floor-rule-result-2026-09-24.md), 2026-09-24; [#123](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/123), [#134](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/134) |
| **A contrast on this population is carried by a handful of rows** | Jackknife influence on the Phase 1 TESS contrast: the fewest most-influential rows whose removal reverses the sign is **1 for recall @1% FPR, 4 for pAUC, 6 for ROC-AUC**, and the single most influential row is 1.21x the whole recall contrast. The count scales with the contrast, and this one is ~0.13x the MDE, so it is a caution about small margins rather than a claim about large ones | [Stage 1](experiments/p2-stage-1-instrument-2026-09-17.md) §5, 2026-09-17 |
| **The follow-up region is the region the evidence cannot see** | No candidate metric detects a degradation confined to the top of the ranking: best z is **0.97 at the top 5%, 1.09 at 10%, 1.36 at 20%**, all against a 1.96 significance bar, averaged over 25 realisations. The console exists to order candidates for follow-up, and every architecture comparison in the record was blind on exactly that axis. This is a property of how few labelled rows land there, not of the metric, and no choice among metrics repairs it — only injection-recovery, which sets its own n | [P2.1](experiments/p2-1-power-analysis-2026-09-14.md) §4, 2026-09-14 |
...
| **The centroid flag is miscalibrated, and its scale is not sigma** | `BEB_THRESHOLD_SIGMA = 3.0` on `centroid_snr` fires on 43.5% of the labelled population and on **27.6% of confirmed planets**, so the console's Centroid Shift card tells more than a quarter of genuine planets that the flux may come from a neighbour. The statistic itself is informative — AUC **0.7339** for predicting a false positive — but its scale is not: **1.21% of targets exceed 1000σ** and the maximum is **10,436σ**, because `extract_centroid_features` guards its denominator only with `sigma > 0`. A threshold fitted on top of that inherits it, so the cut was measured and deliberately left unchanged; the denominator gets a floor first. Separately, 767 of 4,685 bulk-scored candidates (16.4%) have no measurable centroid at all | [centroid flag calibration](experiments/centroid-flag-calibration-2026-09-13.md), 2026-09-13; [#22](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/22) |
| ~~**Three view-set artefacts drift from their DVC pointers**~~ **RESOLVED 2026-09-11** | `viewset.npz`, `viewset_tfrecords` and `viewset_scalars.parquet` on disk differed from what `dvc pull` returned; stage 10.5 trained on the on-disk bytes. Six artefacts were `dvc add`ed to the bytes on disk and pushed, so `dvc pull` now returns what the record was trained on — `viewset.npz` 64,289,921 → 294,083,971 bytes, the dead 301/31 resolution giving way to the 2001/201 one. The superseded set survives only in git history; `dvc checkout` was never the other option, since the on-disk bytes were in no cache. Decision #31 | `dvc status -c`, 2026-09-11; [#31](https://github.com/OliverDonaldson/exoplanet-hunter-v2/issues/31) |
